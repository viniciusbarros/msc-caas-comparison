...
class Runner:
    ...
    def __init__(self) -> None:
        ...
        self.scenarios = [
            ...
            },
            {
                ...
                "start": "cd ../../terraform/aws && terraform plan -out plan -var 'cpu={cpu}' -var 'memory={memory}' && terraform apply -auto-approve",
    ...
    def run_aws(self, scenario):
        ...
        for conf in scenario.get('configs', []):
            ...
            logging.info('Sleeping to give AWS enough time to register the network interfaces')
            time.sleep(30)
            networks = json.loads(self.run_command(scenario.get('get_networks')))
            url = 'http://' + networks['NetworkInterfaces'][0]['Association']['PublicDnsName']
            ...
            # Stopping Container
            logging.info(f"Destroying AWS ECS Fargate w/ CPU: {cpu} and Memory: {memory}")
            self.run_command(scenario.get('stop'))
